...
# define variables -----------------------------------------------------------------------------------------------------------------
PYTHON_BIN="/opt/Carme/Carme-Vendors/mambaforge/envs/carme-backend/bin/python"
...
CARME_NODE_FS=$(get_variable CARME_NODE_FS "${CONFIG_FILE}")
CARME_LOCAL_SSD_PATH=$(get_variable CARME_NODE_SSD_PATH "${CONFIG_FILE}")
CARME_LOGINNODE_IP=$(get_variable CARME_FRONTEND_IP "${CONFIG_FILE}")
CARME_BACKEND_SERVER=$(get_variable CARME_BACKEND_NODE "${CONFIG_FILE}")
CARME_BACKEND_PORT=$(get_variable CARME_BACKEND_PORT "${CONFIG_FILE}")
CARME_SCRIPTS_PATH="/opt/Carme/Carme-Scripts"
CARME_START_SSHD=$(get_variable CARME_NODE_SSHD "${CONFIG_FILE}")
CARME_VERSION=0.9.9
CARME_URL=$(get_variable CARME_FRONTEND_URL "${CONFIG_FILE}")
CARME_TMPDIR=$(get_variable CARME_NODE_TMP_PATH "${CONFIG_FILE}")

[[ -z ${CARME_NODE_FS} ]] && die "CARME_NODE_FS not set"
...
log "distributed file system: ${CARME_NODE_FS}"

if [[ "$(hostname -s)" == "${MASTER_NODE}" && "${CARME_NODE_FS}" == "yes" ]];then
  CARME_NODEID="0"
elif [[ "${CARME_NODE_FS}" == "no" ]];then
  CARME_NODEID="0"
fi
...
export CARME_JOB_FILE_OUT=${LOGDIR}/${SLURM_JOB_ID}.out
export CARME_JOB_FILE_ERR=${LOGDIR}/${SLURM_JOB_ID}.err
" > "${JOBDIR}/bashrc"
