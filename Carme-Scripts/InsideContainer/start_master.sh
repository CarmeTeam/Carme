...
X11Forwarding no
PrintMotd no
AcceptEnv LANG LC_* CUDA* ENVIRONMENT GIT* GPU_DEVICE_ORDINAL HASH JUPYTER_DATA LD_LIBRARY_PATH SINGULARITY* SLURM* S_COLORS TBDIR XGD_RUNTIME_DIR
AllowUsers ${USER}
...
" >> ${SSHDIR}/sshd_config_${SLURM_JOB_ID}
  ...
  rm ${HOME}/.ssh/known_hosts
		...
		echo "SendEnv LANG LC_* CUDA* ENVIRONMENT GIT* GPU_DEVICE_ORDINAL HASH JUPYTER_DATA LD_LIBRARY_PATH SINGULARITY* SLURM* S_COLORS TBDIR XGD_RUNTIME_DIR
HashKnownHosts yes
