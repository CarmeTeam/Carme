...
function carme_mpirun () {
  eval `ssh-agent`
  ssh-add ${HOME}/.ssh/id_rsa_${SLURM_JOB_ID}
  /opt/anaconda3/bin/mpirun -bind-to none -map-by slot -x NCCL_DEBUG=INFO -x LD_LIBRARY_PATH -x HOROVOD_MPI_THREADS_DISABLE=1 -x PATH --mca plm rsh  --mca ras simulator --display-map --wdir ~/tmp --mca btl_openib_warn_default_gid_prefix 0 --mca orte_tmpdir_base ~/tmp --tag-output ${@}
  eval `ssh-agent -k`
}
#-----------------------------------------------------------------------------------------------------------------------------------
...
# redefine horovodrun --------------------------------------------------------------------------------------------------------------
function carme_horovodrun () {
  eval `ssh-agent`
  ssh-add ${HOME}/.ssh/id_rsa_${SLURM_JOB_ID}
  /opt/anaconda3/bin/horovodrun ${@}
  eval `ssh-agent -k`
}
#-----------------------------------------------------------------------------------------------------------------------------------
